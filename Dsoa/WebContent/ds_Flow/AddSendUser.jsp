...
//	email :	fuushikaden@yahoo.com
		...
		/*换人发送*/
		function ReUserSend(){
			if(document.SendList.UList.value==""){
				alert("请选择发送人员");
				//hideSending();
				return;
			}
			document.getElementById("sendtype").value = "1";
			showSending();
			...
			if (<%=dsoap.tools.ConfigurationSettings.AppSettings("分级保护版本")%> == 1 && <%=strMj%> > 0){
				...
				if(isSendMsg && isSendMsg.checked){
		...
		function AddSend(){
			...
			showSending();
			document.getElementById("sendtype").value = "2";
			document.all("TxtPriSend").value = "";
			if (<%=dsoap.tools.ConfigurationSettings.AppSettings("分级保护版本")%> == 1 && <%=strMj%> > 0){
	   			var sulist=document.all.UList.value.split(";");
	   			var strUid = "";
	...
	<body background="../images/bg_body_right.gif" topMargin="0"
		...
		<form id="SendList" name="SendUserList" action="AddSending.action"
			method="POST">
		...
		<div id="sending"
			style="z-index: 10; padding: 0 20 0 20px; visibility: hidden; position: absolute; top: 220px;">
				...
				<td>
					...
					.getTreeHTML1()%></div>
									...
									<table border="0" cellSpacing="0" cellPadding="0"
										...
										<tr>
											<td>
												<input class="user_tree_search" id="searchText" />
											</td>
											<td>
										...
										<tr>
											<td>
												<div style="height: 20px;">
													&nbsp;提醒<input type="checkbox" name="isSendMsg"  checked="checked"/>
												</div>
												<textarea id="SMS" style="width: 100%; height: 65px;"><%=sms%></textarea>
											</td>
										</tr>
										<%
											}
			...
			<tr>
				<td>
					<table cellSpacing="0" cellPadding="0" width="100%" border="0"
						...
						<tr>
							<td align="right">
								<div id="subBu" onclick="AddSend();" class="select_user_send">
									补发
								</div>
							</td>
							<td align="right">
								<div id="subBu" onclick="ReUserSend();" class="select_user_send">
									换人
								</div>
							</td>
	...
	window.onload = function(){
		try{
			...
			**/
			win.setWH({width:500,height:600});
